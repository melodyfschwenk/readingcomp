...
/****************************************************
 * WIAT → Central Sync Bridge
 * Mirrors WIAT into the master workbook (Sessions, Task Progress,
 * Session Events, Video Tracking) without changing WIAT’s own sheets.
 ****************************************************/

const CENTRAL_SYNC = {
  ENABLED: true,
  // ⚠️ Put your **central Spatial Cognition workbook ID** here:
  // e.g., '1AbCDeF...'
  SPREADSHEET_ID: 'PUT_MASTER_SPREADSHEET_ID_HERE',
  TASK_NAME: 'Reading Comprehension Task' // must match master’s required task name
};
...
// If you maintain a PID→Session Code mapping in the CENTRAL workbook:
// Create sheet "Session Map" with headers: PID | Session Code | Email (optional)
function wiat_central__getSessionCode(css, pid){
  var map = wiat_central__ensureSheet(css, 'Session Map', ['PID','Session Code','Email']);
  var vals = map.getDataRange().getValues();
  for (var i=1;i<vals.length;i++){
    if (String(vals[i][0]) === String(pid)) return String(vals[i][1] || pid);
  }
  // Fallback: use PID as Session Code so logging still works
  return String(pid);
}

function wiat_central__touchSession(pid, timestamp){
  central_(function(css){
    ...
    var code = wiat_central__getSessionCode(css, pid);
    ...
    if (!row){
      ...
      wiat_central__setByHeader(sh,row,'Participant ID',pid);
...
function wiat_central__logEvent(pid, type, details, timestamp){
  central_(function(css){
...
function toSeconds_(val){
  var n = Number(val) || 0;
  // If it looks like "minutes", convert; otherwise assume it's already seconds
  return (n > 0 && n < 1000) ? Math.round(n * 60) : Math.round(n);
}

function wiat_central__logTask(pid, eventType, opts){
  central_(function(css){
